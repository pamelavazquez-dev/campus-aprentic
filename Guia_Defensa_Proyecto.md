...
🛡️ **Defensa Oficial:** "Efectivamente, una arquitectura puramente técnica (*Layered Architecture*) escala mal. Por eso hemos dejado asentada en nuestra documentación (ver README) la ruta de migración estructural hacia **Feature-Sliced Design (FSD)**. En lugar de dividir el código por 'qué hace', lo encapsularemos por 'a qué pertenece' (el dominio de negocio). Así, la *feature* `lecciones` tendrá sus propios componentes, hooks y modelos confinados. Esto nos garantiza **Alta Cohesión** y **Bajo Acoplamiento**."
